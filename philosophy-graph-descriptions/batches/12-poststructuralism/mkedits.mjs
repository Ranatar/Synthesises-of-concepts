...
const HERE = byBirth.filter(p=>p.birth>=1925&&p.birth<=1931).map(p=>p.id);
...
const FL = JSON.parse(fs.readFileSync('batch12-flags.json','utf8'));
...
fs.writeFileSync('edits/batch-12-poststructuralism.json', JSON.stringify({meta:{batch:'12',
  title:'Внутрисистемные связи: Делёз, Фуко, Деррида, Рорти (последний внутрисистемный заход)',
  spec:'descriptions-revision-spec.md',specRevision:3,target:'philosophy_graph.html',
  targetMd5:'5bd40d692b0f8aeadcc6f79389c81901',created:'2026-08-09',layer:'inner',
