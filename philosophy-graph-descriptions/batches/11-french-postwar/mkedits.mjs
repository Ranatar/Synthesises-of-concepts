...
const HERE = byBirth.filter(p=>p.birth>=1897&&p.birth<=1908).map(p=>p.id);
...
const FL = JSON.parse(fs.readFileSync('batch11-flags.json','utf8'));
...
fs.writeFileSync('edits/batch-11-french-postwar.json', JSON.stringify({meta:{batch:'11',
  title:'Внутрисистемные связи: Батай, Гадамер, Лакан, Сартр, Левинас, Мерло-Понти, Куайн',
  spec:'descriptions-revision-spec.md',specRevision:3,target:'philosophy_graph.html',
  targetMd5:'72368aa696bbba0df9a42717ef08cc6b',created:'2026-08-09',layer:'inner',
