...
const HERE = byBirth.filter(p=>p.birth>=1833&&p.birth<=1856).map(p=>p.id);
...
const FL = JSON.parse(fs.readFileSync('batch08-flags.json','utf8'));
...
fs.writeFileSync('edits/batch-08-fin-de-siecle.json', JSON.stringify({meta:{batch:'08',
  title:'Внутрисистемные связи: Дильтей, Брентано, Пирс, Ницше, Фреге, Мейнонг, Фрейд',
  spec:'descriptions-revision-spec.md',specRevision:3,target:'philosophy_graph.html',
  targetMd5:'933b98098a51b61d24466d1abba62551',created:'2026-08-09',layer:'inner',
