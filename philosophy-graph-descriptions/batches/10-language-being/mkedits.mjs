...
const HERE = byBirth.filter(p=>p.birth>=1874&&p.birth<=1891).map(p=>p.id);
...
const FL = JSON.parse(fs.readFileSync('batch10-flags.json','utf8'));
...
fs.writeFileSync('edits/batch-10-language-being.json', JSON.stringify({meta:{batch:'10',
  title:'Внутрисистемные связи: Кассирер, Витгенштейн, Хайдеггер, Карнап',
  spec:'descriptions-revision-spec.md',specRevision:3,target:'philosophy_graph.html',
  targetMd5:'b9f2093e70905a716296fe370017b4bd',created:'2026-08-09',layer:'inner',
